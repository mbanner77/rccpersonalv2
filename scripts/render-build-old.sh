...
if [ -n "${DATABASE_URL:-}" ]; then
  # For new/empty databases or schema drift, use db push to sync schema directly
  # This is simpler and more reliable than complex migration sanitization
  log "Using prisma db push to sync schema (accepts data loss for new DB)"
  set +e
...
# Note: patterns handle optional schema prefix like "public".
remove_patterns = [
    r'CREATE\s+TYPE[^;]*"TaskStatus"[^;]*;',
    r'DROP\s+TYPE[^;]*"TaskStatus"[^;]*;',
    r'ALTER\s+TYPE[^;]*"TaskStatus"[^;]*;',
    r'CREATE\s+TYPE[^;]*"LifecycleOwnerRole"[^;]*;',
    r'DROP\s+TYPE[^;]*"LifecycleOwnerRole"[^;]*;',
    r'ALTER\s+TYPE[^;]*"LifecycleOwnerRole"[^;]*;',
