...
export default async function DashboardPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [settings, employees, lastImport, taskStats, upcomingReminders, ticketStats, assetStats, vehicleStats] = await Promise.all([
    ...
    // HR Ticket statistics
    (async () => {
      try {
        const [total, open, inProgress] = await Promise.all([
          (db as unknown as { hRTicket: { count: () => Promise<number> } }).hRTicket.count(),
          (db as unknown as { hRTicket: { count: (args: { where: { status: string } }) => Promise<number> } }).hRTicket.count({ where: { status: "OPEN" } }),
          (db as unknown as { hRTicket: { count: (args: { where: { status: string } }) => Promise<number> } }).hRTicket.count({ where: { status: "IN_PROGRESS" } }),
        ]);
        return { total, open, inProgress };
      } catch { return { total: 0, open: 0, inProgress: 0 }; }
    })(),
    // Asset statistics
    (async () => {
      try {
        const [total, assigned, available, pendingTransfers] = await Promise.all([
          (db as unknown as { asset: { count: () => Promise<number> } }).asset.count(),
          (db as unknown as { asset: { count: (args: { where: { assignedToEmployeeId: { not: null } } }) => Promise<number> } }).asset.count({ where: { assignedToEmployeeId: { not: null } } }),
          (db as unknown as { asset: { count: (args: { where: { assignedToEmployeeId: null } }) => Promise<number> } }).asset.count({ where: { assignedToEmployeeId: null } }),
          (db as unknown as { assetTransfer: { count: (args: { where: { status: string } }) => Promise<number> } }).assetTransfer.count({ where: { status: "PENDING" } }),
        ]);
        return { total, assigned, available, pendingTransfers };
      } catch { return { total: 0, assigned: 0, available: 0, pendingTransfers: 0 }; }
    })(),
    // Vehicle statistics
    (async () => {
      try {
        const [total, assigned] = await Promise.all([
          (db as unknown as { vehicle: { count: () => Promise<number> } }).vehicle.count(),
          (db as unknown as { vehicle: { count: (args: { where: { assignedToId: { not: null } } }) => Promise<number> } }).vehicle.count({ where: { assignedToId: { not: null } } }),
        ]);
        return { total, assigned };
      } catch { return { total: 0, assigned: 0 }; }
    })(),
  ]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-zinc-50 via-white to-zinc-100 dark:from-zinc-900 dark:via-zinc-900 dark:to-zinc-800">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="flex items-center gap-3">
              ...
              <span className="rounded-full bg-gradient-to-r from-indigo-500 to-purple-500 px-3 py-1 text-xs font-medium text-white">
                {now.toLocaleDateString("de-DE", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}
              </span>
            </div>
            <p className="mt-1 text-sm text-zinc-500">
              Übersicht über {activeEmployees.length} Mitarbeitende • {birthdaysToday > 0 ? `🎂 ${birthdaysToday} Geburtstag${birthdaysToday > 1 ? "e" : ""} heute` : "Keine Geburtstage heute"} • {hits7.length > 0 ? `🎉 ${hits7.length} Jubiläum${hits7.length > 1 ? "en" : ""} diese Woche` : "Keine Jubiläen diese Woche"}
            </p>
          </div>
        ...
        <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {/* HR Tickets */}
          <Link href="/tickets" className="group relative overflow-hidden rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg dark:border-zinc-700 dark:bg-zinc-800">
            <div className="absolute -right-4 -top-4 h-24 w-24 rounded-full bg-blue-100/50 transition-transform group-hover:scale-125 dark:bg-blue-900/20" />
            <div className="relative">
              ...
              <div className="mt-3 text-xs font-medium uppercase tracking-wide text-zinc-500">HR-Tickets</div>
              <div className="mt-1 text-3xl font-bold text-zinc-900 dark:text-white">{ticketStats.open}</div>
              <div className="mt-1 flex items-center gap-2 text-xs text-zinc-500">
                <span className="rounded-full bg-amber-100 px-2 py-0.5 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">{ticketStats.inProgress} in Bearbeitung</span>
              </div>
            </div>
          </Link>

          {/* Hardware/Assets */}
          <Link href="/hardware" className="group relative overflow-hidden rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg dark:border-zinc-700 dark:bg-zinc-800">
            <div className="absolute -right-4 -top-4 h-24 w-24 rounded-full bg-cyan-100/50 transition-transform group-hover:scale-125 dark:bg-cyan-900/20" />
            <div className="relative">
              ...
              <div className="mt-3 text-xs font-medium uppercase tracking-wide text-zinc-500">Hardware</div>
              <div className="mt-1 text-3xl font-bold text-zinc-900 dark:text-white">{assetStats.total}</div>
              <div className="mt-1 flex items-center gap-2 text-xs text-zinc-500">
                <span className="text-emerald-600 dark:text-emerald-400">{assetStats.available} verfügbar</span>
                {assetStats.pendingTransfers > 0 && (
                  <span className="rounded-full bg-orange-100 px-2 py-0.5 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400">{assetStats.pendingTransfers} Übertragungen</span>
                )}
              </div>
            </div>
          </Link>
          ...
          {/* Fleet */}
          <Link href="/fleet" className="group relative overflow-hidden rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg dark:border-zinc-700 dark:bg-zinc-800">
            <div className="absolute -right-4 -top-4 h-24 w-24 rounded-full bg-violet-100/50 transition-transform group-hover:scale-125 dark:bg-violet-900/20" />
            <div className="relative">
              ...
              <div className="mt-3 text-xs font-medium uppercase tracking-wide text-zinc-500">Fuhrpark</div>
              <div className="mt-1 text-3xl font-bold text-zinc-900 dark:text-white">{vehicleStats.total}</div>
              <div className="mt-1 flex items-center gap-2 text-xs text-zinc-500">
                <span className="text-violet-600 dark:text-violet-400">{vehicleStats.assigned} zugewiesen</span>
                <span className="text-zinc-400">·</span>
                <span>{vehicleStats.total - vehicleStats.assigned} verfügbar</span>
              </div>
            </div>
          </Link>

          {/* Active Employees */}
          <Link href="/employees" className="group relative overflow-hidden rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg dark:border-zinc-700 dark:bg-zinc-800">
            <div className="absolute -right-4 -top-4 h-24 w-24 rounded-full bg-indigo-100/50 transition-transform group-hover:scale-125 dark:bg-indigo-900/20" />
            <div className="relative">
              ...
              <div className="mt-3 text-xs font-medium uppercase tracking-wide text-zinc-500">Mitarbeitende</div>
              <div className="mt-1 text-3xl font-bold text-zinc-900 dark:text-white">{activeEmployees.length}</div>
              <div className="mt-1 flex items-center gap-2 text-xs text-zinc-500">
                <span className="text-emerald-600 dark:text-emerald-400">Aktiv</span>
                <span className="text-zinc-400">·</span>
                <span>{exitedEmployees.length} ausgetreten</span>
              </div>
            </div>
          </Link>
        </div>
        ...
        <div className="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-800">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <Link href="/tickets" className="flex items-center gap-2 rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2.5 text-sm font-medium text-zinc-700 transition hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:border-blue-600 dark:hover:bg-blue-900/20">
              ...
              <Link href="/hardware" className="flex items-center gap-2 rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2.5 text-sm font-medium text-zinc-700 transition hover:border-cyan-300 hover:bg-cyan-50 hover:text-cyan-700 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:border-cyan-600 dark:hover:bg-cyan-900/20">
              ...
              <Link href="/fleet" className="flex items-center gap-2 rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2.5 text-sm font-medium text-zinc-700 transition hover:border-violet-300 hover:bg-violet-50 hover:text-violet-700 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:border-violet-600 dark:hover:bg-violet-900/20">
              ...
              <Link href="/admin/settings" className="flex items-center gap-2 rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2.5 text-sm font-medium text-zinc-700 transition hover:border-zinc-400 hover:bg-zinc-100 hover:text-zinc-800 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:border-zinc-500 dark:hover:bg-zinc-700">
